const fields = ["lpType", "tone", "offerName", "purpose", "audience", "mainOffer", "cta"];

const options = {
  lpType: ["販売型", "予約・申込型", "登録・リード獲得型", "アプリ・SaaS型", "ブランド・世界観型", "キャラクター・IP紹介型", "イベント告知型", "記事・コンテンツ誘導型"],
  tone: ["静かで信頼感がある", "やさしく親しみやすい", "高級感がある", "かわいい", "雑誌風", "ファンタジー寄り", "シンプルで実用的"],
  purpose: ["購入につなげる", "予約につなげる", "登録につなげる", "問い合わせにつなげる", "読了につなげる", "フォローにつなげる", "参加申込につなげる"],
  audience: ["初めて見る人", "比較検討している人", "既存ファン", "SNSから来た人", "初心者", "作品世界を知りたい人"],
  cta: ["購入する", "予約する", "無料で受け取る", "登録する", "問い合わせる", "続きを読む", "フォローする", "参加する"]
};
...
const templates = {
  "販売型": [
    ["ファーストビュー", "商品名、誰向けか、得られる変化をすぐ見せる。", ["商品メインビジュアル", "使用後のイメージ画像"]],
    ["悩み・欲しい状態", "購入前の迷いや欲しい未来を整理する。", ["悩みを静かに示す背景", "比較しやすい図解"]],
    ["商品紹介", "何を販売しているか、内容物、特徴を明確にする。", ["商品単体画像", "内容物の俯瞰画像"]],
    ["使い方・使用シーン", "購入後の使い方を具体的に見せる。", ["生活や作業の中で使う画像"]],
    ["価格・プラン", "価格、内容、購入判断に必要な情報を整理する。", ["価格表背景", "特典イメージ"]],
  ...
  "予約・申込型": [
    ...
    ["サービスでできること", "提供内容と範囲を明確にする。", ["サービス利用シーン", "手元や空間の写真"]],
    ["利用の流れ", "申し込み前の不安を減らす。", ["ステップ図", "受付から完了までの流れ"]],
    ["選ばれる理由", "信頼材料、こだわり、姿勢を整理する。", ["静かな信頼感のある背景"]],
...
function buildSection([name, role, assets], index) {
  return [
    `[${index + 1}. ${name}]`,
    `役割: ${role}`,
    "見出し: ",
...
function buildStructure() {
  ...
  if (!lpType) return "";
  const sections = templates[lpType] || templates["予約・申込型"];
  const data = {
    tone: clean(getValue("tone")),
    offerName: clean(getValue("offerName")),
    purpose: clean(getValue("purpose")),
    audience: clean(getValue("audience")),
    mainOffer: clean(getValue("mainOffer")),
    cta: clean(getValue("cta"))
  };
